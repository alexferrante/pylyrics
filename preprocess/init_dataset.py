...
def create_dataset(path, num_files):
  ...
  if not os.path.isfile('coefficients/batch_stft_%s.npz' %path):
    ...
    log_stft_mixed_norm = [np.log(src_mx+data_constants.eps) for src_mx in stft_mixed_norm]
    log_stft_bg_norm = [np.log(src_bg+data_constants.eps) for src_bg in stft_bg_norm]
    log_stft_vc_norm = [np.log(src_vc+data_constants.eps) for src_vc in stft_vc_norm]
    ...
    log_mfcc_mixed = [np.log(src_mx+absmin_mx+data_constants.eps) for src_mx in mfcc_mixed]
    log_mfcc_bg = [np.log(src_bg+absmin_bg+data_constants.eps) for src_bg in mfcc_bg]
    log_mfcc_vc = [np.log(src_vc+absmin_vc+data_constants.eps) for src_vc in mfcc_vc]	

    # Normalized STFT
    batch_stft_mixed = [to_batch(src_mixed) for src_mixed in log_stft_mixed_norm]
    batch_stft_bg = [to_batch(src_bg) for src_bg in  log_stft_bg_norm]
    batch_stft_vc = [to_batch(src_vc) for src_vc in log_stft_vc_norm]

    # MFCC
    batch_mfcc_mixed = [to_batch(src_mixed) for src_mixed in log_mfcc_mixed]
    batch_mfcc_bg = [to_batch(src_bg) for src_bg in log_mfcc_bg]
    batch_mfcc_vc = [to_batch(src_vc) for src_vc in log_mfcc_vc]
...
def to_batch(input):
  ...
  n_coef = input.shape[1]

  if(n_frames % data_constants.BATCH_SIZE != 0):
    rem = data_constants.BATCH_SIZE - (n_frames % data_constants.BATCH_SIZE)
    zed = np.zeros((1, int(rem) * n_coef)).reshape((-1, n_coef))
    input = np.vstack((input, zed))

  batches = np.reshape(input, (-1, data_constants.BATCH_SIZE, n_coef))
  return batches

def to_coef(batches, frames=409):
  n_batches, n_frames, n_coefs = batches.shape
  coefs = np.reshape(batches, (-1, n_coefs))
  return coefs
